
import { Recipe } from "./recipe";
import { Ingredient } from "./ingredient";

 export var RECIPES: Recipe[] = [
	new Recipe(0, 
		'Chicken Cacciatore',
		'<p>Hsdaas sfsasdf asfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf.</p> \
        <p>A sfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf. \
        Esafasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf. Hsfsdf asfsa \
        safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf.</p> \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf...</p>',
		[
			new Ingredient('Chicken', 1),
			new Ingredient('Salt', 1),
			new Ingredient('Salad', 2),
		],
        '/assets/uploads/chickencacciatore.jpg'
	),
	new Recipe(1, 
		'Au Grautin Potatos',
		'<p>Gsaf sfsasdf asfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf.</p> \
        <p>A sfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf. \
        Esafasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf. Hsfsdf asfsa \
        safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf.</p> \
        <p>Sfsasdf asfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf...</p>',
		[
			new Ingredient('Potatos', 4),
			new Ingredient('Milk', 1),
			new Ingredient('Butter', 4),
			new Ingredient('Mayonaise', 2),
		],
        '/assets/uploads/potatos.jpg'
	),
    new Recipe(2, 
		'Steak Tar Tar',
		'<p>A fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf.</p> \
        <p>A sfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf. \
        <p>Sfsasdf asfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf...</p>',
		[
			new Ingredient('Steak', 1),
			new Ingredient('Onions', 1),
		],
        '/assets/uploads/steak.jpg'
	),
    new Recipe(3, 
		'Persian Eggplant',
		'<p>Gsaf sfsasdf asfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf.</p> \
        <p>A sfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf. \
        Esafasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf. Hsfsdf asfsa \
        safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf.</p> \
        <p>Sfsasdf asfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf.</p>',
		[
			new Ingredient('Large eggplant', 3),
			new Ingredient('Onions', 1),
            new Ingredient('Butter (stick)', 1),
            new Ingredient('Mushrooms', 1),
            new Ingredient('Garlic', 1),
		],
        '/assets/uploads/eggplant.jpg'
	),
    new Recipe(4, 
		'Buttermilk Chicken',
		'<p>Gsaf sfsasdf asfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf.</p> \
        <p>A sfdsadfsf, asfsd d fsdfsa fsdf sfdsafsadf s sd sd sdf sa fasasfsadfsfasdfsfsaf \
        safsafsaf. Bdd safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf. \
        Esafasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf. Hsfsdf asfsa \
        safasfggd sadfsa ded er ret gf d asfasfsdfasfdddfsf asf asf sad fasdf safaf sadf s \
        as safsaasfs. M as fasfsafasfdsf sdf asf asf as f asdf asdf assadf sad sad asdfasdf.</p>',
		[
			new Ingredient('Large eggplant', 3),
			new Ingredient('Onions', 1),
            new Ingredient('Butter (stick)', 1),
		],
        '/assets/uploads/chicken.jpg')
];
